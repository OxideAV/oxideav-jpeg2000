//! CxtVLC tables transcribed verbatim from ISO/IEC 15444-15:2019
//! Annex C (FDIS pages 34-53).
//!
//! `CxtVLC_table_0` is used for quads in the *first* line-pair of a
//! code-block (q < QW); `CxtVLC_table_1` is used for all subsequent
//! quads. Each entry is a 7-tuple `(cq, rho, u_off, emb_k, emb_1, w, l_w)`
//! (the spec uses the Greek letter symbols `ρ_q`, `u_q^off`,
//! `ε^k_q`, `ε^1_q` — they are spelled out here for plain-ASCII
//! readability), decoded by matching the codeword bits against `w`
//! (read LSB-first from the VLC bit-stream — see Annex C, paragraph
//! "field w represents the codeword as a little-endian integer,
//! meaning that the LSB of the integer w appears first") with length
//! `l_w`.
//!
//! The tables are kept in their natural order from the spec text;
//! [`super::cxt_vlc::CxtVlcTable`] builds an indexed lookup by
//! `(c_q, codeword)` from these flat arrays.

/// One row of an Annex C CxtVLC table: `(c_q, ρ_q, u_off, ε^k, ε^1, w, l_w)`.
pub type Entry = (u8, u8, u8, u8, u8, u8, u8);

/// `CxtVLC_table_0` from §C (FDIS pages 34-44).
pub const CXT_VLC_TABLE_0: &[Entry] = &[
    ...
    // ROUND-6.5 fix: spec entry (FDIS Annex C line 1985 of the
    // searchable PDF) is `{0, 0xC, 0x1, 0xC, 0xC, 0x17, 7}` — the
    // `ε^1_q` (emb_1) field is `0xC`, not `0x0`. The earlier
    // transcription dropped a hex digit, leaving the decoder with
    // `ibit = 0` for samples j=2,3 of a quad whose right column should
    // carry an "magnitude is at the top of the encodable range" bit.
    // The bug only manifested on HL_R1 (the smallest non-trivial HF
    // codeblock in the §12.2 trace fixture), causing right-column
    // pixels to come back at half magnitude.
    ...
    // ROUND-6.5 fix: spec entry (FDIS Annex C line 2352 of the
    // searchable PDF) is `{6, 0xD, 0x1, 0x5, 0x5, 0x33, 7}` — the
    // codeword length is 7 bits, not 6. The original transcription
    // dropped the trailing zero bit and would have had the matcher
    // greedy-accept at length 6 (cwd 0x33) before reaching the
    // 7-bit entry. Confirmed against OpenJPEG `t1_ht_luts.h` —
    // `vlc_tbl0[(6 << 7) | 51]` = `0x5573`, decoding to length 7.
...
/// `CxtVLC_table_1` from §C (FDIS pages 44-53).
pub const CXT_VLC_TABLE_1: &[Entry] = &[
